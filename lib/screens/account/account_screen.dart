...
class _AccountScreenState extends ConsumerState<AccountScreen> {
  ...
  Future<void> _loadUserDetails() async {
    ...
    if (widget.role == 'Merchant') {
      // ── Merchant: load profile from Firestore via provider ──
      if (ref.read(merchantProvider).merchant == null) {
        await ref.read(merchantProvider.notifier).loadMerchantProfile();
      }
      if (mounted) {
        setState(() => _isLoading = false);
      }
    } else {
      // ── Customer: existing SharedPreferences / DatabaseService path ──
      ...
      if (firstName == null ||
          ...
          firstName == 'Merchant' ||
          firstName == 'Sam') {
        debugPrint('AccountScreen: Name missing, triggering sync...');
        await DatabaseService().syncUserProfileToPreferences(role: widget.role);
      }

      if (mounted) {
        final fName = prefs.getString('firstName') ?? 'Sam';
        final lName = prefs.getString('lastName') ?? 'William';
        final mail = prefs.getString('email') ?? 'sam@email.com';
  ...
  Widget build(BuildContext context) {
    ...
    // Show merchant error as SnackBar
    ...
    final String displayName = widget.role == 'Merchant'
        ? (merchantState.merchant?.outletName ?? 'Loading...')
        : (user.name ?? 'Loading...');
    final String displayEmail = widget.role == 'Merchant'
        ? (merchantState.merchant?.ownerEmail ?? 'Loading...')
        : (user.email ?? 'Loading...');
